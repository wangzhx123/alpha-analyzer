...
class FillRateAnalyzer(BaseAnalyzer):
    ...
    def _get_fill_data(self, split_alpha_df: pd.DataFrame, realtime_pos_df: pd.DataFrame,
                       ti_filter=None, ticker_filter=None):
        ...
        for i in range(len(times) - 1):
            ...
            for _, alpha_row in alphas.iterrows():
                ...
                # Find positions
                curr_pos = pos_current[(pos_current['alphaid'] == alphaid) & (pos_current['ticker'] == ticker)]
                next_pos = pos_next[(pos_next['alphaid'] == alphaid) & (pos_next['ticker'] == ticker)]
                
                if not curr_pos.empty and not next_pos.empty:
                    pos_change = next_pos['realtime_pos'].iloc[0] - curr_pos['realtime_pos'].iloc[0]
                    fill_rate = pos_change / target_alpha if abs(target_alpha) > 1e-6 else 0
                    
                    fill_data.append({
                        'time_from': t_current, 'time_to': t_next,
                        'alphaid': alphaid, 'ticker': ticker,
                        'target_alpha': target_alpha, 'position_change': pos_change,
                        'fill_rate': fill_rate
                    })
    ...
    def _analyze_overview(self, incheck_alpha_df: pd.DataFrame, merged_df: pd.DataFrame,
        ...
        summary = (f"Total trades: {len(df)} | Analyzable: {len(finite_df)} | "
                  f"Mean fill rate: {finite_df['fill_rate'].mean():.3f}")
        
        details = (f"Best performer: {best_performer['ticker']} ({best_performer['fill_rate']:.3f})\\n"
    ...
    def _analyze_time_event(self, ti: int, incheck_alpha_df: pd.DataFrame, merged_df: pd.DataFrame,
        ...
        # Group by ticker
        ticker_stats = finite_df.groupby('ticker')['fill_rate'].agg(['mean', 'count']).round(3)
        
        summary = (f"ti={ti}: {len(ticker_stats)} tickers analyzed | "
    ...
    def _analyze_ticker_timeline(self, ticker: str, incheck_alpha_df: pd.DataFrame, merged_df: pd.DataFrame,
        ...
        # Group by time
        time_stats = finite_df.groupby('time_from')['fill_rate'].agg(['mean', 'count']).round(3)
        
        summary = (f"ticker={ticker}: {len(time_stats)} time periods | "
                  f"Overall mean: {finite_df['fill_rate'].mean():.3f}")
        
        details = "Timeline performance:\\n"
        for time_period, stats in time_stats.iterrows():
            details += f"  ti={time_period}: {stats['mean']:.3f} ({stats['count']} trades)\\n"
        
        # Timeline plot
        fig, ax = plt.subplots(1, 1, figsize=(10, 6))
        time_stats.index = range(len(time_stats))  # Sequential index for plotting
        ax.plot(time_stats.index, time_stats['mean'], marker='o', linewidth=2, markersize=6)
        ax.axhline(y=1.0, color='red', linestyle='--', label='Perfect Fill')
    ...
    def _analyze_deep(self, ti: int, ticker: str, incheck_alpha_df: pd.DataFrame, merged_df: pd.DataFrame,
        ...
        summary = (f"ti={ti}, ticker={ticker}: {len(finite_df)} trades | "
        ...
        for _, trade in finite_df.iterrows():
            details += (f"  {trade['alphaid']}: target={trade['target_alpha']:.0f}, "
        ...
        ax1.scatter(finite_df['target_alpha'], finite_df['position_change'], s=100, alpha=0.7)
        max_val = max(finite_df['target_alpha'].max(), finite_df['position_change'].max())
        min_val = min(finite_df['target_alpha'].min(), finite_df['position_change'].min())
        ...
        if len(finite_df) > 1:
            finite_df.set_index('alphaid')['fill_rate'].plot(kind='bar', ax=ax2, color='lightgreen')
        else:
        ...
        # Position changes
        ax3.bar(range(len(finite_df)), finite_df['position_change'], color='orange', alpha=0.7, label='Actual')
        ax3.bar(range(len(finite_df)), finite_df['target_alpha'], alpha=0.5, color='blue', label='Target')
