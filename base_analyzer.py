...
class BaseAnalyzer(ABC):
    ...
    def analyze_overview(
        ...
        return self._analyze_overview(
            incheck_alpha_df, merged_df, split_alpha_df, realtime_pos_df, market_df
        )
    ...
    def analyze_time_event(
        ...
        if not self.supports_time_event():
            return None
        return self._analyze_time_event(
            ti, incheck_alpha_df, merged_df, split_alpha_df, realtime_pos_df, market_df
        )
    ...
    def analyze_ticker_timeline(
        ...
        if not self.supports_ticker_timeline():
            return None
        return self._analyze_ticker_timeline(
            ticker,
            incheck_alpha_df,
            merged_df,
            split_alpha_df,
    ...
    def _analyze_ticker_timeline(
        ...
        raise NotImplementedError(
            f"{self.name} does not support ticker timeline analysis"
        )
