...
class MergeEngineChecker(BaseChecker):
    ...
    def check(
        ...
        try:
            ...
            merge_issues = self._validate_pm_to_groups(
                in_check_alpha_df, merged_alpha_df
            )
            issues.extend(merge_issues)

            # Phase 2: Validate Merged Groups → Split to Traders
            split_issues = self._validate_groups_to_traders(
                merged_alpha_df, split_alpha_df
            )
    ...
    def _validate_pm_to_groups(self, in_check_df, merged_df):
        ...
        merged_groups = (
            merged_df.groupby(["time", "ticker"])["volume"].sum().reset_index()
        )
        ...
        for _, merged_row in merged_groups.iterrows():
            ...
            pm_entries = in_check_df[
                (in_check_df["time"] == time_val) & (in_check_df["ticker"] == ticker)
            ]

            if AlphaAnalyzer.is_previous_day_position(
                time_val
                ...
                pm_details = ", ".join(
                    [f"{pm}: {vol}" for pm, vol in pm_breakdown.items()]
                )
    ...
    def _validate_groups_to_traders(self, merged_df, split_df):
        ...
        split_groups = (
            split_df.groupby(["time", "ticker"])["volume"].sum().reset_index()
        )
