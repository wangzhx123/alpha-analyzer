...
class PMConstraintChecker(BaseChecker):
    ...
    def check(
        ...
        time_events = sorted(
            [
                t
                for t in split_alpha_df["time"].unique()
                if not AlphaAnalyzer.is_previous_day_position(t)
            ]
        )
        ...
        for time_event in time_events:
            ...
            for ticker in time_alphas["ticker"].unique():
                ...
                total_current_pos = (
                    ticker_positions["realtime_pos"].sum()
                    if not ticker_positions.empty
                    else 0
                )
                total_available_sellable = (
                    ticker_positions["realtime_avail_shot_vol"].sum()
                    if not ticker_positions.empty
                    else 0
                )
                    ...
                    if required_sell > total_available_sellable:
                        ...
                        for _, pos_row in ticker_positions.iterrows():
                            trader_details.append(
                                f"{pos_row['alphaid']}:{pos_row['realtime_avail_shot_vol']}"
                            )
        ...
        if violations:
            ...
            details_lines.append(
                f"Found {total_violations} T+1 settlement rule violations:"
            )
            ...
            for time_event in sorted(violations_by_time.keys()):
                ...
                details_lines.append(
                    f"  time={time_event}: {len(time_violations)} violations"
                )
                ...
                for v in time_violations[:5]:  # Show first 5 violations per time
                    ...
                    details_lines.append(
                        f"      Trader sellable: {', '.join(v['trader_details'])}"
                    )

                if len(time_violations) > 5:
                    details_lines.append(
                        f"    ... and {len(time_violations) - 5} more violations"
                    )
            ...
            total_checked = len(
                merged_df[
                    ~merged_df["time"].apply(AlphaAnalyzer.is_previous_day_position)
                ]
            )
            return CheckResult(
                checker_name=self.name,
